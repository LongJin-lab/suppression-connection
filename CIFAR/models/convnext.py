...
def MeanStd(**kwargs):
    if dataset == 'cifar10':
        ...
        num_classes = 1000
    return IMAGE_DEFAULT_MEAN,IMAGE_DEFAULT_STD,num_classes

IMAGE_DEFAULT_MEAN,IMAGE_DEFAULT_STD,num_classes = MeanStd()

def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': num_classes, 'input_size': (3, 224, 224), 'pool_size': (7, 7),
...
class ConvNeXt(nn.Module):
    ...
    def __init__(
            self, in_chans=3, num_classes=1000, global_pool='avg', output_stride=32, patch_size=4,
            depths=(3, 3, 9, 3), dims=(96, 192, 384, 768),  ls_init_value=1e-6, conv_mlp=False, stem_type='patch',
            head_init_scale=1., head_norm_first=False, norm_layer=None, drop_rate=0., drop_path_rate=0., givenA=[1,0], givenB=[1, 0], ini_stepsize=1, settings='',PL=1, ini_block_shift=None
    ):    
        super().__init__()
