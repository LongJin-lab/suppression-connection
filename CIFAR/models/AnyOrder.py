...
class SamBlock(nn.Module):
    expansion = 1
    def __init__(self, in_planes, planes, pre_planes, coesA, coesB, coes_stepsize, steps=3, stride=1, coe_ini=1, fix_coe=False, given_coe=[1.0 / 3, 5.0 / 9, 1.0 / 9, 16.0 / 9]):        
    ...
    def forward(self, x, coesA, coesB, coes_stepsize):  
...
class ZeroSBlock(nn.Module): 
    ...
    def __init__(self, in_planes, planes, last_res_planes, l_last_res_planes,  stride=1, coe_ini=1, fix_coe=False, coes_stepsize=1, given_coe=[1.0/3, 5.0/9, 1.0/9, 16.0/9], downsample=None):
    ...
    def forward(self, x, last_res, l_last_res): 
        ...
        if not (isinstance(last_res,int) or isinstance(l_last_res,int)):
            ...
            x = torch.mul(self.coes_stepsize, torch.mul(self.b_0, F_x_n)) + torch.mul(self.a_0, residual) + torch.mul(self.a_1, last_res) + torch.mul(self.a_2, l_last_res)
        else:
            x = F_x_n
        l_last_res = last_res
        last_res = residual 
...
class ZeroSBottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, last_res_planes, l_last_res_planes,  stride=1, coe_ini=1, fix_coe=False, coes_stepsize=1, given_coe=[1.0/3, 5.0/9, 1.0/9, 16.0/9]):
...
class ZeroSNet_Tra(nn.Module):
    ...
    def __init__(self, block, layers, coe_ini=1, share_coe=False, pretrain=False, num_classes=num_cla, stochastic_depth=False, PL=1.0, noise_level=0.001,
        ...
        # self.coes_stepsize =nn.Parameter(torch.Tensor(1).uniform_(1, 1))
        blocks = []
        self.coes = []
...
class ZeroSBlockAnysettings(nn.Module):
    expansion = 1
    def __init__(self, in_planes, planes, pre_planes, coesA, coesB, coes_stepsize, steps=3, stride=1, coe_ini=1, fix_coe=False, given_coe=[1.0 / 3, 5.0 / 9, 1.0 / 9, 16.0 / 9], settings='', Layer_idx=0, coesDecay=1):        
    ...
    def forward(self, x, pre_features, pre_acts, coesA, coesB, coes_stepsize):  
                                    
        # ## residual = pre_features[0]
        residual = x

        #             if 'ConvStride2Share' in self.settings or 'ConvStride2ResLikeShare' in self.settings:
        #                 if self.in_planes != self.planes:
        ...
        #             elif 'ConvStride2Learn' in self.settings or 'ConvStride2ResLike' in self.settings:
        #                 # print('ConvStride2Learn')
        #                 if self.in_planes != self.planes:
        #                     residual = self.downsample_x_Learn(residual)  
        #                 for i in range(self.steps-1):
        ...
        if 'ExpDecay' in self.settings:
            x_ExpDecay = torch.exp(self.coesDecay*self.Layer_idx*coes_stepsize)*(x-self.coes_bias_inner)
        else:
            x_ExpDecay = x
        if not 'DSfirst' in self.settings:     
               
            if 'BnReluConv' in self.settings:
                F_x_n = self.bn1(x_ExpDecay)
                F_x_n = self.relu(F_x_n)
                F_x_n = self.conv1(F_x_n)
                F_x_n = self.bn2(F_x_n)
                F_x_n = self.relu(F_x_n)
                F_x_n = self.conv2(F_x_n)
            elif 'BnReluConvBn' in self.settings:
                F_x_n = self.bn1(x_ExpDecay)
                F_x_n = self.relu(F_x_n)
                F_x_n = self.conv1(F_x_n)
                F_x_n = self.bn2(F_x_n)
        ...
        if not (self.has_ini_block):
            ...
            if 'ExpDecay' in self.settings:
                # print('self.coesDecay',self.coesDecay)
                F_x_n = torch.exp(-self.coesDecay*self.Layer_idx*coes_stepsize) * F_x_n -self.coesDecay*residual+self.coes_bias_outer
                
                # F_x_n =  torch.mul((1-torch.sigmoid(self.coesBalance)), torch.exp(-self.coesDecay*self.Layer_idx*coes_stepsize) * (F_x_n-self.coesDecay*residual) ) +torch.mul(torch.sigmoid(self.coesBalance), F_x_n)
                # print('self.coesDecay, coes_stepsize, torch.sigmoid(self.coesBalance)', self.coesDecay, coes_stepsize, torch.sigmoid(self.coesBalance)  )        
                # F_x_n = torch.mul((1-self.coesDecay), torch.exp(-coes_stepsize*self.Layer_idx) * (F_x_n-residual) )+torch.mul(self.coesDecay,F_x_n)
                
                
                # F_x_n = torch.exp(-self.coesDecay*self.Layer_idx) * (F_x_n-residual)
                # print('self.coesDecay, self.Layer_idx', self.coesDecay.data, self.Layer_idx)            
            ...
            # if coes_stepsize == -1:
            #     coes_stepsize = 1      
            # x =  torch.add( sum_features, torch.mul(coes_stepsize, sum_acts ) )
            x =  torch.add( sum_features, torch.mul(coes_stepsize, -sum_acts ) )
        
            # x =  torch.add( sum_features, torch.mul(self.coes_stepsize, self.coesA[-1].expand_as(F_x_n)*F_x_n) )
            ...
            # if 'ExpDecay' in self.settings:
            #     residual_ExpDecay = torch.exp(self.coesDecay*self.Layer_idx*coes_stepsize)*(residual-self.coes_bias_inner)
            # else:
            ...
            if 'ExpDecay' in self.settings:
                ...
                F_x_n = torch.exp(-self.coesDecay*self.Layer_idx*coes_stepsize) * F_x_n -self.coesDecay*residual+self.coes_bias_outer
                # F_x_n =  torch.mul((1-torch.sigmoid(self.coesBalance)), torch.exp(-self.coesDecay*self.Layer_idx*coes_stepsize) * (F_x_n-self.coesDecay*residual) ) +torch.mul(torch.sigmoid(self.coesBalance), F_x_n)    
            # print('F_x_n, residual',F_x_n.shape, residual.shape)
            x = F_x_n + residual
...
class ZeroSAnysettings(nn.Module):
    ...
    def __init__(self, block, layers, coe_ini=1, givenA=[0.33333333333, 0.55555555556, 0.1111111111], givenB=[0.888888888889, 0.8888888888889, 0], share_coe=False, ini_stepsize=1, pretrain=False, num_classes=num_cla, stochastic_depth=False, PL=1.0, noise_level=0.001,
        ...
        if 'LearnCoe' in self.settings:   
            print('LearnCoe')         
            # self.coesA = nn.ParameterList([nn.Parameter(torch.ones(1)*self.givenA[i], requires_grad=True) for i in range(self.steps)]) 
            self.coesB = nn.ParameterList([nn.Parameter(torch.ones(1)*self.givenB[i], requires_grad=True) for i in range(self.steps)]) 
        else:    
        ...
        if not self.stochastic_depth:
            for i in range(3):
                ...
                print('l',l)
                blocks.append(block(self.in_planes, self.planes[i], self.pre_planes, coesA=self.coesA, coesB=self.coesB,coes_stepsize=self.ini_stepsize, steps=self.steps, stride=self.strides[i], coe_ini=self.coe_ini, settings=self.settings,Layer_idx=Layer_idx, coesDecay=coesDecay))
                ...
                for k in range(1, layers[i]):
                    ...
                    blocks.append(block(self.in_planes, self.planes[i], self.pre_planes, coesA=self.coesA, coesB=self.coesB, coes_stepsize=self.ini_stepsize, steps=self.steps, coe_ini=self.coe_ini, settings=self.settings,Layer_idx=Layer_idx, coesDecay=coesDecay))
...
class ZeroSBlockAny(nn.Module):
    expansion = 1
    def __init__(self, in_planes, planes, pre_planes, coesA, coesB, coes_stepsize, steps=3, stride=1, coe_ini=1, fix_coe=False, given_coe=[1.0 / 3, 5.0 / 9, 1.0 / 9, 16.0 / 9], settings=''):        
        ...
        for pre_pl in self.pre_planes_b:
            if pre_pl <=0:
                ...
                self.is_ini = False
            self.has_ini_block = self.is_ini or self.has_ini_block #wrong
        if 'ConvStride2Learn' in self.settings:
            if not (self.has_ini_block):
                if self.in_planes != self.planes:
                    self.downsample_x = Downsample_clean(self.in_planes, self.planes, 2)
                start_DS = 1        
...
class ZeroSAny_Tra(nn.Module):
    ...
    def __init__(self, block, layers, coe_ini=1, givenA=[0.33333333333, 0.55555555556, 0.1111111111], givenB=[0.888888888889, 0.8888888888889, 0], share_coe=False, ini_stepsize=1, pretrain=False, num_classes=num_cla, stochastic_depth=False, PL=1.0, noise_level=0.001,
        ...
        if not self.stochastic_depth:
            for i in range(3):
                # print('self.pre_planes in net1', self.pre_planes)
                blocks.append(block(self.in_planes, self.planes[i], self.pre_planes, coesA=self.coesA, coesB=self.coesB,coes_stepsize=self.ini_stepsize, steps=self.steps, stride=self.strides[i], coe_ini=self.coe_ini, settings=self.settings))
                ...
                for k in range(1, layers[i]):
                    ...
                    blocks.append(block(self.in_planes, self.planes[i], self.pre_planes, coesA=self.coesA, coesB=self.coesB, coes_stepsize=self.ini_stepsize, steps=self.steps, coe_ini=self.coe_ini, settings=self.settings))
...
class SamNet(nn.Module):
    ...
    def __init__(self, block, layers, steps=3, coe_ini=1, givenA=[0.33333333333, 0.55555555556, 0.1111111111], givenB=[0.888888888889, 0.8888888888889, 0], share_coe=False, ini_stepsize=1, pretrain=False, num_classes=num_cla, stochastic_depth=False, PL=1.0, noise_level=0.001,
        ...
        if not self.stochastic_depth:
            for i in range(3):
                # print('self.pre_planes in net1', self.pre_planes)
                blocks.append(block(self.in_planes, self.planes[i], self.pre_planes, coesA=self.coesA, coesB=self.coesB,coes_stepsize=self.ini_stepsize, steps=self.steps, stride=self.strides[i], coe_ini=self.coe_ini))
                ...
                for k in range(1, layers[i]):
                    ...
                    blocks.append(block(self.in_planes, self.planes[i], self.pre_planes, coesA=self.coesA, coesB=self.coesB, coes_stepsize=self.ini_stepsize, steps=self.steps, coe_ini=self.coe_ini))
...
class MaskBlock(nn.Module):
    expansion = 1
    def __init__(self, input_size, in_planes, planes, pre_planes, coesA, coesB, coes_stepsize, steps=3, stride=1, coe_ini=1, fix_coe=False, given_coe=[1.0 / 3, 5.0 / 9, 1.0 / 9, 16.0 / 9]):        
...
class MaskNet(nn.Module):
    ...
    def __init__(self, block, layers, coe_ini=1, givenA=[0.33333333333, 0.55555555556, 0.1111111111], givenB=[0.888888888889, 0.8888888888889, 0], share_coe=False, ini_stepsize=1, pretrain=False, num_classes=num_cla, stochastic_depth=False, PL=1.0, noise_level=0.001,
        ...
        if not self.stochastic_depth:
            for i in range(3):
                # print('self.pre_planes in net1', self.pre_planes)
                blocks.append(block(self.input_size[i], self.in_planes, self.planes[i], self.pre_planes, coesA=self.coesA, coesB=self.coesB,coes_stepsize=self.ini_stepsize, steps=self.steps, stride=self.strides[i], coe_ini=self.coe_ini))
                if l < steps-1:
                    for j in range(steps-2,0,-1): # steps-1, steps-2, ..., 1
                        self.pre_planes[j] =  self.pre_planes[j-1]
                ...
                for k in range(1, layers[i]):
                    ...
                    blocks.append(block(self.input_size[i], self.in_planes, self.planes[i], self.pre_planes, coesA=self.coesA, coesB=self.coesB, coes_stepsize=self.ini_stepsize, steps=self.steps, coe_ini=self.coe_ini))
...
class ZeroSNet_Opt(nn.Module):
    ...
    def __init__(self, block, layers, coe_ini=-9.0 / 5, fix_coe=True, given_coe=[1.0/3, 5.0/9, 1.0/9, 16.0/9], pretrain=False, num_classes=num_cla, stochastic_depth=False, PL=1.0, noise_level=0.001, noise=False):
        ...
        if not self.stochastic_depth:
            for i in range(3):  
                blocks.append(
                    block(self.in_planes, self.planes[i], self.last_res_planes, self.l_last_res_planes, self.strides[i],
                          coe_ini=self.coe_ini, coes_stepsize=self.coes_stepsize, fix_coe=self.fix_coe, given_coe=self.given_coe))
            ...
            for i in range(3):
                blocks.append(
                    block(self.in_planes, self.planes[i], self.last_res_planes, self.l_last_res_planes, self.strides[i],
                          coe_ini=self.coe_ini, fix_coe=self.fix_coe, coes_stepsize=self.coes_stepsize,
    ...
    def forward(self, x):
        ...
        return x, self.coes, self.coes_stepsize
...
#         # coes_stepsize =nn.Parameter(torch.Tensor(1).uniform_(self.coe_ini, self.coe_ini))
#         # torch.nn.init.constant_(coes_stepsize, self.coe_ini)
#         # coes = torch.cat( (coes, coes_stepsize), 0)
...
#     #              coes_stepsize=1, given_coe=[1.0 / 3, 5.0 / 9, 1.0 / 9, 16.0 / 9], downsample=None):
...
#     def forward(self, x, pre_features, coes, coes_stepsize):  
...
#             # x =  torch.add( sum_features, torch.mul(coes_stepsize, torch.mul(self.coes[-1].data, F_x_n) ) )
#             x =  torch.add( sum_features, torch.mul(coes_stepsize, self.coes[-1].expand_as(F_x_n)*F_x_n) )

#             # pre_features[0] =  torch.add( sum_features, torch.mul(self.coes_stepsize, torch.mul(self.coes[-1], F_x_n) ) )


#                 # x = torch.mul(self.a_0, residual) + torch.mul(
#                 # self.a_1, last_res) + torch.mul(self.a_2, l_last_res)+ torch.mul(self.a_3, l_l_last_res)+torch.mul(self.coes_stepsize, torch.mul(self.b_0, F_x_n))
...
#                 # self.a_1 = -1 / (self.coe)
#                 # self.a_2 = (self.coe + 1) / (4 * self.coe)
#             # x = torch.mul(self.coes_stepsize, torch.mul(self.b_0, F_x_n)) + torch.mul(self.a_0, residual) + torch.mul(
...
#     net = ZeroSAny32settings(givenA=[1.49323762497707, -0.574370781405754, 0.0855838379295368, -0.00445068150085398,
# ], givenB=[0.400601121454727, -1.68484817541425, 1.80393876806197, -2.10313656405320], settings='mnistAllElePlotTraj')#S4O0
    
#     net = ZeroSAny32settings(givenA=[1.49323762497707, -0.574370781405754, 0.0855838379295368, -0.00445068150085398,
# ], givenB=[-2.10313656405320, 5.60787753612393, -7.29272571153819, 3.20453988951669
# ], settings='mnistAllElePlotTraj')#S4O2 ExpDecay
    
    net = ZeroSAny20settings(givenA=[1,0
], givenB=[-1,0
], settings='2Chs_BnReluConv_ConvStride2ResLike_ShareExpDecayLearnDecay_AbsExp_Adam_LREndBN_RestaLayerIdx3')#S4O4
    # net = ZeroSAny32settings(givenA=[1, 0, 0, 0, 0, 0], givenB=[-2.9701388888889, 5.5020833333333,  -6.9319444444444, 5.0680555555556, -1.9979166666667, 0.32986111111111 ], settings='mnistAllElePlotTraj')
    # net = ZeroSAny32settings(givenA=[1, 0], givenB=[-1, 0 ], settings='mnistAllElePlotTraj')    
    ...
    print('df', df)
    save_path = './OverThreeOrders/OverThreeOrders/CIFAR/runs/features/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
