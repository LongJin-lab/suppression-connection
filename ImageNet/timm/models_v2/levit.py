...
class Residual(nn.Module):
    def __init__(self, m, drop, coesA=[], coesB=[], stepsize=1, settings='',Layer_idx=None, coesDecay=None):
        super().__init__()
        self.m = m
        self.drop = drop
    ...
    def forward(self, x):
        ...
        if 'ExpDecay' in self.settings:  
            ...
            if 'InvExp' in self.settings:
                x_k_exp = torch.exp(-self.coesDecay*self.Layer_idx*self.coes_stepsize)*(x)#-self.bias_inner)
            else:
                x_k_exp = torch.exp(self.coesDecay*self.Layer_idx*self.coes_stepsize)*(x)#-self.bias_inner)
        else:
            x_k_exp = x
        ...
        if self.training and self.drop > 0:
            x_k_exp = self.m(x_k_exp) * torch.rand(
                x_k_exp.size(0), 1, 1, device=x.device).ge_(self.drop).div(1 - self.drop).detach()
            
        else:
            x_k_exp = self.m(x_k_exp)
        if 'ExpDecay' in self.settings:
            if 'InvExp' in self.settings:
                x_k_exp = torch.exp(self.coesDecay*self.Layer_idx*self.coes_stepsize) * x_k_exp -self.coesDecay*x#+self.bias_outer
...
class Levit(nn.Module):
    ...
    def __init__(
            ...
            drop_path_rate=0.,
            ini_stepsize=1,
            givenA=[0.33333333333, 0.55555555556, 0.1111111111], givenB=[0.888888888889, 0.8888888888889, 0],
            settings='',Layer_idx=None, IniDecay=None
            ):
        super().__init__()
            ...
            for block_idx in range(dpth):
                ...
                self.blocks.append(
                    Residual(
                        ...
                        drop_path_rate,coesA=self.coesA, coesB=self.coesB, stepsize=self.stepsize, settings=self.settings,Layer_idx=l_, coesDecay=self.coesDecay))
                if mr > 0:
                    h = int(ed * mr)
                    self.blocks.append(
