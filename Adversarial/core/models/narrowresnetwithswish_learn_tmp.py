...
class _Block(nn.Module):
    ...
    def forward(self, x_and_f):
        ...
        elif not 'NoSig' in self.settings:
        ...
        if 'ExpDecay' in self.settings:
            ...
            if 'ExpDecay' in self.settings:
                ...
                if 'InvExp' in self.settings:
                    f_k = torch.exp(self.coesDecay*self.Layer_idx*self.coes_stepsize) * f_k -self.coesDecay*x_k+self.bias_outer
                else:
                    f_k = torch.exp(-self.coesDecay*self.Layer_idx*self.coes_stepsize) * f_k -self.coesDecay*x_k+self.bias_outer
        else:
            if self.has_shortcut:
...
class WideResNet_learn(nn.Module):
    ...
    def __init__(self,
        ...
        if 'ExpDecay' in self.settings and 'LearnDecay' in self.settings:            
            coesDecay =nn.Parameter(torch.ones(1)*(IniDecay), requires_grad=True)
        else:
            coesDecay = nn.Parameter(torch.ones(1)*(IniDecay), requires_grad=False)
...
def create_network():
    net = narrowresnetwithswish_learn(name='nrn-70-1-swish-learn',coesB=[-1.91666667,  1.33333333, -0.41666667],learn=True,IniRes=False,Mask=False,IniCh=4,settings='ExpDecayLearnDecayLearnBal',IniDecay=-4)
