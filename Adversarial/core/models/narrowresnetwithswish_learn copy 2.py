            ...
            if i < nb_ini_block:
                ...
                block.append(
                    _Block(i == 0 and in_planes or out_planes, 
                        out_planes,
                        i == 0 and stride or 1,
                        activation_fn=activation_fn,coesB=coesB,input_size=input_size,learn=learn,IniRes=IniRes,Mask=Mask,IniCh=IniCh, settings=settings,Layer_idx=i, coesDecay=coesDecay)
                )
...
class WideResNet_learn(nn.Module):
    ...
    def __init__(self,
                 ...
                 coesB=[0,0,0],learn=False,IniRes=False,Mask=False,IniCh=16,settings=''):
        super().__init__()
        
        self.settings = settings
        if 'ExpDecay' in self.settings and 'LearnDecay' in self.settings:            
            self.coesDecay =nn.Parameter(torch.ones(1)*1)#, requires_grad=True)
        else:
            self.coesDecay = nn.Parameter(torch.ones(1)*1, requires_grad=False)
        ...
        self.layer = nn.Sequential(
            _BlockGroup(num_blocks, self.set_ini, self.input_size[0], num_channels[0], num_channels[1], 1,
                        activation_fn=activation_fn,coesB=coesB,learn=learn,IniRes=IniRes,Mask=Mask,IniCh=IniCh, settings=settings, coesDecay=self.coesDecay),
...
def narrowresnetwithswish_learn(name, dataset='cifar10', num_classes=10, device='cpu', coesB=[0,0,0],learn=False,IniRes=False,Mask=False,IniCh=16,settings=''):
    """
...
def create_network():
    net = narrowresnetwithswish_learn(name='nrn-70-1-swish-learn',coesB=[-1.91666667,  1.33333333, -0.41666667],learn=True,IniRes=False,Mask=False,IniCh=4,settings='ExpDecayLearnDecayLearnBal')
