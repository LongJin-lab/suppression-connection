...
class _BlockIni(nn.Module):
    ...
    """
    def __init__(self, in_planes, out_planes, stride, activation_fn=nn.ReLU,coesB=[0,0,0],input_size=0,learn=False,IniRes=False,Mask=False,IniCh=4):
        super().__init__()
...
class _Block(nn.Module):
    ...
    Arguments:
        ...
        activation_fn (nn.Module): activation function.
    """
    def __init__(self, in_planes, out_planes, stride, activation_fn=nn.ReLU,coesB=[0,0,0],input_size=0,learn=False,IniRes=False,Mask=False,IniCh=4,settings=''):
        super().__init__()
        self.batchnorm_0 = nn.BatchNorm2d(in_planes, momentum=0.01)
        self.relu_0 = activation_fn(inplace=False)#True)
        # We manually pad to obtain the same effect as `SAME` (necessary when `stride` is different than 1).
...
class _BlockGroup(nn.Module):
    ...
    Arguments:
        ...
        activation_fn (nn.Module): activation function.
    """
    def __init__(self, num_blocks, nb_ini_block, input_size, in_planes, out_planes, stride, activation_fn=nn.ReLU, coesB=[0,0,0],learn=False,IniRes=False,Mask=False,IniCh=4,settings=''):
        super().__init__()
        block = []
...
class WideResNet_learn(nn.Module):
    ...
    def __init__(self,
                 ...
                 coesB=[0,0,0],learn=False,IniRes=False,Mask=False,IniCh=4,settings=''):
        super().__init__()
        ...
        self.layer = nn.Sequential(
            _BlockGroup(num_blocks, self.set_ini, self.input_size[0], num_channels[0], num_channels[1], 1,
                        activation_fn=activation_fn,coesB=coesB,learn=learn,IniRes=IniRes,Mask=Mask,IniCh=IniCh),
...
def narrowresnetwithswish_learn(name, dataset='cifar10', num_classes=10, device='cpu', coesB=[0,0,0],learn=False,IniRes=False,Mask=False,IniCh=4,settings=''):
    """
    ...
    if 'cifar100' in dataset:
        return WideResNet_learn(num_classes=num_classes, depth=depth, width=widen, activation_fn=_ACTIVATION[act_fn], 
                          mean=CIFAR100_MEAN, std=CIFAR100_STD, coesB=coesB,learn=learn,IniRes=IniRes,Mask=Mask,IniCh=IniCh,settings=settings)
    elif 'svhn' in dataset:
        return WideResNet_learn(num_classes=num_classes, depth=depth, width=widen, activation_fn=_ACTIVATION[act_fn], 
                          mean=SVHN_MEAN, std=SVHN_STD, coesB=coesB,learn=learn,IniRes=IniRes,Mask=Mask,IniCh=IniCh,settings=settings)
    return WideResNet_learn(num_classes=num_classes, depth=depth, width=widen, activation_fn=_ACTIVATION[act_fn], coesB=coesB,learn=learn,IniRes=IniRes,Mask=Mask,IniCh=IniCh,settings=settings)

def create_network():
    net = IniChresnetwithswish_learn(name='nrn-28-1-swish-learn',coesB=[-1.91666667,  1.33333333, -0.41666667],learn=False,IniRes=False,Mask=False,IniCh=4,settings='ExpDecayLearnDecayLearnBal')
