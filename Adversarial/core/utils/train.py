...
class Trainer(object):
    ...
    def __init__(self, info, args):
        ...
        if 'learn' in args.model:
            self.model = create_model(args.model, args.normalize, info, device, args.coesB, args.learn, args.IniRes, args.Mask, args.IniCh, args.settings,args.IniDecay)
        else: 
            self.model = create_model(args.model, args.normalize, info, device)#, args.coesB)
        self.params = args
        ...
        if self.params.pretrained_file is not None:
            ...
            self.load_model(os.path.join(self.params.log_dir, self.params.pretrained_file, 'weights-last.pt'))
    ...
    def init_optimizer(self, num_epochs):
        ...
        """
        self.optimizer_NoCoe = torch.optim.SGD([{'params':[ param for name, param in self.model.named_parameters() if ('Fix' not in name and 'coes' not in name)]}, 
        ...
        if 'SGD' in self.params.settings:
            ...
            {'params': (p for name, p in self.model.named_parameters() if 'coes' in name), 'weight_decay': 0.,},
    ...
    def init_scheduler(self, num_epochs):
        ...
        if self.params.scheduler == 'cyclic':
            ...
            self.scheduler_NoCoe = torch.optim.lr_scheduler.OneCycleLR(self.optimizer_NoCoe, max_lr=self.params.lr, pct_start=0.25,steps_per_epoch=update_steps, epochs=int(num_epochs))
            if self.optimizer_Coe:
                self.scheduler_Coe = torch.optim.lr_scheduler.OneCycleLR(self.optimizer_Coe, max_lr=self.params.CoesLR, pct_start=0.25,steps_per_epoch=update_steps, epochs=int(num_epochs))
            else:
                self.scheduler_Coe = None
            
        elif self.params.scheduler == 'step':
            self.scheduler_NoCoe = torch.optim.lr_scheduler.MultiStepLR(self.optimizer_NoCoe, gamma=0.1, milestones=[100, 105])  
            if self.optimizer_Coe:
                self.scheduler_Coe = torch.optim.lr_scheduler.MultiStepLR(self.optimizer_Coe, gamma=0.1, milestones=[100, 105])  
            else:
                self.scheduler_Coe = None
                        
        elif self.params.scheduler == 'cosine':
            self.scheduler_NoCoe = CosineLR(self.optimizer_NoCoe, max_lr=self.params.lr, epochs=int(num_epochs))
            if self.optimizer_Coe:
                self.scheduler_Coe = CosineLR(self.optimizer_Coe, max_lr=self.params.CoesLR, epochs=int(num_epochs))
            else:
                self.scheduler_Coe = None
        elif self.params.scheduler == 'cosinew':
            self.scheduler_NoCoe = torch.optim.lr_scheduler.OneCycleLR(self.optimizer_NoCoe, max_lr=self.params.lr, pct_start=0.025, total_steps=int(num_epochs))
            if self.optimizer_Coe:
                self.scheduler_Coe = torch.optim.lr_scheduler.OneCycleLR(self.optimizer_Coe, max_lr=self.params.CoesLR, pct_start=0.025, total_steps=int(num_epochs))
            else:
                self.scheduler_Coe = None
        else:
            self.scheduler_NoCoe = None
            self.scheduler_Coe = None
    ...
    def standard_loss(self, x, y):
        ...
        Standard training.
        """
        self.optimizer_NoCoe.zero_grad()
        if self.optimizer_Coe:
            self.optimizer_Coe.zero_grad()
        out = self.model(x)
    ...
    def mart_loss(self, x, y, beta):
        ...
        MART training.
        """
        loss, batch_metrics = mart_loss(self.model, x, y, self.optimizer_NoCoe, self.optimizer_Coe,step_size=self.params.attack_step, 
                                        epsilon=self.params.attack_eps, perturb_steps=self.params.attack_iter, 
                                        beta=beta, attack=self.params.attack)
        return loss, batch_metrics  
